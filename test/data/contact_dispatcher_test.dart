  ...
  group(HttpContactDispatcher, () {
    group('send', () {
      ...
      test('reports nothing when the endpoint accepts it', () async {
        expect(await _answering(204).send(_draft), isNull);
      });

      test('reads any 2xx as accepted, not only the 204 the function sends', () async {
        expect(await _answering(200).send(_draft), isNull);
      });

      test('reads a 400 as the endpoint refusing what was typed', () async {
        expect(await _answering(400).send(_draft), DispatchFailure.rejected);
      });

      test('reads a 429 as a rate limit', () async {
        expect(await _answering(429).send(_draft), DispatchFailure.rateLimited);
      });

      test('reads a 502 as delivery being down', () async {
        expect(await _answering(502).send(_draft), DispatchFailure.mailer);
      });
      ...
      test('reads a request that never left as a lost connection', () async {
        ...
        expect(await dispatcher.send(_draft), DispatchFailure.network);
      });

...
| Chrome | `lib/ui/core/view/*.dart` | `AppShell` takes the routed `child`; `SiteHeader`, `SiteFooter` read `AppShellState` | `lib/ui/core/view/app_shell.dart` | rarely — the shell is what every route renders inside |
